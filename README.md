...
4. Результат сохраняется в новый файл Excel (исходник не трогается).
...
## Выходной файл Excel
Скрипт сохраняет результат как новый файл в папку внутри проекта:
...
- `--sleep-s` (default `0.2`) — пауза между запросами к вакансиям
...
- `POST /api/v1/export/auto` — тело JSON: `queries`, `pages`, `per_page`, те же kw/token/sleep; ответ — `.xlsx`.
